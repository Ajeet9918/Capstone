...
const mockBins = Array.from({ length: 20 }).map((_, i) => {
  ...
  const predictedFullTime =
    fill >= 100 ? 0 : Math.max(1, Math.floor((100 - fill) / fillRate));
  ...
  return {
    ...
    fill,
    status: fill > 85 ? "Critical" : fill > 70 ? "High" : "Normal",
    updated: new Date(
      Date.now() - Math.floor(Math.random() * 7200000),
    ).toLocaleString(),
    fillRate,
    predictedFullTime,
    predictedFullDate: new Date(
      Date.now() + predictedFullTime * 60 * 60 * 1000,
    ).toLocaleString(),
    isAnomaly,
    anomalyType,
    coordinates,
  ...
  function renderBins(bins = mockBins) {
    ...
    bins.forEach((b) => {
      ...
      const predictionText =
        b.fill >= 100
          ? "Full"
          : `${b.predictedFullTime}h (${b.predictedFullDate})`;
      const anomalyText = b.isAnomaly
        ? `<span class="badge anomaly">${b.anomalyType.replace("_", " ")}</span>`
        : "Normal";
      tr.innerHTML = `<td>${b.id}</td><td>${b.location}</td><td>${b.fill}%</td><td><span class="badge">${b.status}</span></td><td>${b.updated}</td><td>${predictionText}</td><td>${anomalyText}</td><td><div class="bin-actions"><button class="edit" data-id="${b.id}">Edit</button><button class="delete" data-id="${b.id}">Delete</button></div></td>`;
    ...
    function filterBins() {
      ...
      if (filterZone.value) {
        filtered = filtered.filter((b) =>
          b.location.includes(filterZone.value),
        );
      }
      if (binSearch.value) {
        filtered = filtered.filter(
          (b) =>
            b.id.toLowerCase().includes(binSearch.value.toLowerCase()) ||
            b.location.toLowerCase().includes(binSearch.value.toLowerCase()),
        );
      }
      renderBins(filtered);
    }
...
function showBinModal(mode, binId = null) {
  ...
  if (!modal) {
    ...
    if (cancelBin)
      cancelBin.addEventListener("click", () => modal.classList.remove("show"));
...
if (dl) {
  dl.addEventListener("click", () => {
    let csv =
      "id,location,fill,status,updated,predicted_full_time,predicted_full_date\n";
    mockBins.forEach(
      (b) =>
        (csv += `${b.id},${b.location},${b.fill},${b.status},${b.updated},${b.predictedFullTime}h,${b.predictedFullDate}\n`),
    );
    const blob = new Blob([csv], { type: "text/csv" });
    const a = document.createElement("a");
    a.href = URL.createObjectURL(blob);
    a.download = "bins-report.csv";
    a.click();
...
if (changePassword) {
  changePassword.addEventListener("click", () => {
    alert(
      "Change password functionality would open a modal or redirect to password change page.",
    );
  });
}
...
if (deleteAccount) {
  deleteAccount.addEventListener("click", () => {
    if (
      ...
      alert(
        "Account deletion would require additional confirmation and processing.",
      );
    }
  });
}
...
if (searchInput) {
  searchInput.addEventListener("input", () => {
    ...
    if (query) {
      const matchingBins = mockBins.filter(
        (b) =>
...
if (fillChartCanvas) {
  ...
  // Get top 10 bins by fill level for cleaner visualization
  const topBins = [...mockBins].sort((a, b) => b.fill - a.fill).slice(0, 10);

  const fillLevels = topBins.map((b) => b.fill);
  const labels = topBins.map((b) => b.id.replace("BIN-", ""));

  // Chart dimensions
  const width = fillChartCanvas.width;
  const height = fillChartCanvas.height;
  const padding = 40;
  const chartHeight = height - padding * 2;
  const chartWidth = width - padding * 2;
  const barWidth = Math.floor(chartWidth / fillLevels.length) - 12;
  const gap = 12;
  ...
  const labelColor = isDarkMode ? "#6b7280" : "#6b7280";
  const baselineColor = isDarkMode ? "#374151" : "#e5e7eb";

  // Draw bars
  let x = padding + gap / 2;
  fillLevels.forEach((fill, i) => {
    const barHeight = (fill / 100) * chartHeight;
    ...
    if (fill > 85) {
      ...
      gradient.addColorStop(1, "#008a52");
    }
    ...
    ctx.fillText(fill + "%", x + barWidth / 2, y - 6);
...
if (themeToggleBtn && fillChartCanvas) {
  const originalThemeHandler = themeToggle.onclick;
  themeToggleBtn.addEventListener("click", () => {
    // Small delay to let theme change complete
    setTimeout(() => {
      const ctx = fillChartCanvas.getContext("2d");
      const topBins = [...mockBins]
...
function renderAlerts(filter = "all", search = "") {
  ...
  if (filter === "critical")
    filtered = filtered.filter((b) => b.status === "Critical");
  else if (filter === "high")
    filtered = filtered.filter((b) => b.status === "High");
  else if (filter === "normal")
    filtered = filtered.filter((b) => b.status === "Normal");
  if (search) {
    filtered = filtered.filter(
      (b) =>
  ...
  if (filtered.length === 0) {
    alertsContainer.innerHTML =
      '<div class="card">No alerts matching criteria</div>';
  } else {
    filtered.forEach((c) => {
  ...
  // Update counts
  if (criticalCount)
    criticalCount.textContent = mockBins.filter(
      (b) => b.status === "Critical",
    ).length;
  if (highCount)
    highCount.textContent = mockBins.filter((b) => b.status === "High").length;
  if (normalCount)
    normalCount.textContent = mockBins.filter(
      (b) => b.status === "Normal",
    ).length;
...
if (refreshAlerts) {
  refreshAlerts.addEventListener("click", () => {
    renderAlerts(
      alertFilter ? alertFilter.value : "all",
      alertSearch ? alertSearch.value : "",
    );
  });
}
...
if (generateReport) {
  generateReport.addEventListener("click", () => {
    ...
    let content =
      '<div class="report-summary"><h4>Report Generated</h4><p>Type: ' +
      (reportType ? reportType.value : "Bins") +
      "</p><p>Format: " +
      (reportFormat ? reportFormat.value : "CSV") +
      "</p></div>";
    content +=
      '<table class="report-table"><thead><tr><th>ID</th><th>Location</th><th>Fill</th><th>Status</th><th>Predicted Full</th></tr></thead><tbody>';
    mockBins.forEach((b) => {
...
function optimizeRoute(bins) {
  ...
  const highPriority = bins
    .filter((b) => b.priority === 3)
    .sort((a, b) => b.fill - a.fill);
  const mediumPriority = bins
    .filter((b) => b.priority === 2)
    .sort((a, b) => b.fill - a.fill);
  const lowPriority = bins
...
if (predictionSummary) {
  ...
  const urgentBins = mockBins.filter(
    (b) => b.predictedFullTime <= 24 && b.predictedFullTime > 0,
  ).length;
  ...
  predictionSummary.innerHTML = `
    ...
    <p><strong>Optimized Route:</strong> ${optimizedRoute
      .slice(0, 5)
      .map((b) => b.id)
      .join(" → ")}${optimizedRoute.length > 5 ? "..." : ""}</p>
  `;
}
